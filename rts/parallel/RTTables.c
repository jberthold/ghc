...
StgTSO* findTSO(StgWord processId, StgWord id) {
  ...
  if (p == NULL ) {
    IF_PAR_DEBUG(procs,
                 debugBelch("findTSO %" FMT_Word
                            ": Unknown process %" FMT_Word ".\n",
                            id, processId));
    return NULL;
  }

  IF_PAR_DEBUG(procs,
      debugBelch("Searching thread %" FMT_Word
                 " in process %" FMT_Word " (%d threads).\n",
                 id, processId, p->tsos);
      printAllThreads());
  ...
  if (processId != proc) {
    IF_PAR_DEBUG(procs,
                 debugBelch("findTSO: wrong process ID %" FMT_Word ". Thread %"
                            FMT_Word " claims it belongs to process %"
                            FMT_Word ".\n",
                            processId, id, proc));
    return NULL;
  }
...
void removeTSO(StgWord id) {
  ...
  if (proc == (StgWord) NULL) {
    ...
    return ; // virtually "removed", shrug (might be a finalizer?)
  }
  p = findProcess(proc);
  if (p == NULL) {
    IF_PAR_DEBUG(procs,
  ...
  while(inp != NULL) {
    ...
    if (temp->closure == NULL) { // a garbage inport, remove it
      ...
      if (!isNoPort(temp->sender)) { // known sender?
        IF_PAR_DEBUG(ports,
                     debugBelch("sending terminate to sender (%d,%"
                                FMT_Word ", %" FMT_Word ")\n",
                                temp->sender.machine,
                                temp->sender.process,
                                temp->sender.id));
