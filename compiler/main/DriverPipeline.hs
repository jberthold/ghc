...
-- wrapper script calling the binary. We need this only in a parallel
-- setup which expects the binary in a central location shared by all PEs.
-- PVM does exactly this. The MPI version could transfer the program to
-- all nodes before starting.
--
...
--  MPI: rename program, generate start script (using mpirun)
	...
	current <- getCurrentDirectory
	    ...
	    copypath = if WayParPvm `elem` (wayNames dflags)-- is for PVM
	...
	Exception.catchIO (do ps <- getPermissions executable
                              writeFile script_name 
                                  (mkWrapperScript os (wayNames dflags) 
...
mkWrapperScript :: OS -> [WayName] -> String -> String -> String
mkWrapperScript OSMinGW32 ways executable _ -- executable_base unused
    = let trace_processing | not (WayEventLog `elem` ways) = []
                           | otherwise =
               ["If dotrace Then",
                "    tracefile2 = \"\" & tracefile & \".zip\" & \"\"",
                "    tracefile3 = \"\" & tracefile & \".parevents\" & \"\"",
                "    executableInit = Replace(executable,\".exe\",\"\")",
                "    Set fso = CreateObject(\"Scripting.FileSystemObject\")",
                "    If fso.FileExists(tracefile2) Then fso.DeleteFile tracefile2",
                "    If fso.FileExists(tracefile3) Then fso.DeleteFile tracefile3",
                "    \'create empty zip file",
                "    Set zipFso = fso.CreateTextFile(tracefile2)",
                "    zipFso.Write Chr(80) & Chr(75) & Chr(5) & Chr(6) & String(18, 0)",
                "    zipFso.Close",
                "    Set zipFSO = Nothing",
                "    Set app = CreateObject(\"shell.application\")",
                "    For i = 1 to nprocessors",
                "        app.NameSpace(tracefile2).MoveHere \"\" & executableInit & \"#\" & i &\".eventlog\" & \"\"",
                "        On Error Resume Next",
                "        Do Until i <= app.NameSpace(tracefile2).Items.Count",
                "            Wscript.Sleep(100)",
                "        Loop",
                "    Next",
                "    fso.MoveFile tracefile2 , tracefile3",
                "    Wscript.Echo \"Trace Postprocessing Done\"",
                "End If"]
      in unlines $
 ["' This script is automatically generated: DO NOT EDIT!!!",
  ...
  "Set reN  = new regexp 'Create RegExp object",
  "Set reqp = new regexp \'Create RegExp object",
  "",
  "' process the args first",
  "tracefile       = executable",
  "in_RTS_Args     = false",
  "reN.Pattern     = \"-N(\\d+)\"",
  "reqp.Pattern    = \"-qp(\\d+)\"",
  "nprocessors     = 1",
  ...
  "tracefile = tracefile & \"_\" & ARG",
  "If Arg = \"+RTS\" Then",
  "    in_RTS_Args = true",
  "ElseIf Arg = \"-RTS\" Then",
  "    in_RTS_Args = false",
  "End If",
  "IF reN.Test(Arg) And in_RTS_args Then",
  "    nprocessors = Int(Replace(Arg,\"-N\",\"\"))",
  "ElseIf reqp.Test(Arg) And in_RTS_args Then",
  "    nprocessors = Int(Replace(Arg,\"-qp\",\"\"))",
  "ElseIf Arg = \"-l\" Then",
  "    dotrace = true",
  "    non_script_args = non_script_args & \" \" & Arg",
  "ElseIf Arg = \"-ls\" Then",
  "    dotrace = true",
  "    non_script_args = non_script_args & \" \" & Arg",
  "Else",
...
mkWrapperScript _ ways executable executable_base 
		 ...
		 | otherwise = panic "Something wrong with compiler ways"
           trace_processing | not (WayEventLog `elem` ways) = []
            ...
            ["# trace copying or archive creation ",
             ...
             "    system(\"if which zip &> /dev/null; then zip -gm $trcfile.parevents $ENV{'HOME'}/$executable_base\\#[0123456789]*.eventlog .; else  mv $ENV{'HOME'}/$executable_base\\#[0123456789]*.eventlog $trcfile\\_parevents; fi\");",
             "  }",
             "}",
             ""]
      in unlines $
 ["eval 'exec perl -S $0 ${1+\"$@\"}'",
  ...
  "",
-- bug: argument "0" impossible like this:
--  "args: while ($a = shift(@ARGV)) {",
-- better use @ARGV (no. of arguments) as criteria
  "args: while (@ARGV) {",
  "    $a = shift(@ARGV);",
  "    $trcfile = $trcfile . \"_\" . $a; ",
  "    if ( $a eq '+RTS' ) {",
