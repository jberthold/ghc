...
wayOpts _ WayParPvm = do
        upd $ addOptP "-D__PARALLEL_HASKELL__"
        upd $ addOptc "-DPARALLEL_RTS"
        upd $ addOptc "-DUSE_PVM" -- use PVM, PVMComm.c
{- Fix linker flags! Were added inside DriverPipeline...
        upd $ addOptl "-L${PVM_ROOT}/lib/${PVM_ARCH}"
        upd $ addOptl "-lpvm3"
        upd $ addOptl "-lgpvm3" -}
wayOpts _ WayParMPI = do
        upd $ addOptP "-D__PARALLEL_HASKELL__"
        upd $ addOptc "-DPARALLEL_RTS"
        upd $ addOptc "-DUSE_MPI" -- use MPI, MPIComm.c
wayOpts _ WayParCp = do
        upd $ addOptP "-D__PARALLEL_HASKELL__"
        upd $ addOptc "-DPARALLEL_RTS"
        upd $ addOptc "-DUSE_COPY" -- use shared memory regions, CpComm.c
wayOpts _ WayPar = do
